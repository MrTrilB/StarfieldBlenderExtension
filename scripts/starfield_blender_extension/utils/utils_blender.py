...
from ..operators import CapsuleGenGeoNode as capsule_gen
from ..operators import PlaneGenGeoNode as plane_gen
...
def ThirdPartyFolderPath():
	utils_path = _update_path(os.path.dirname(os.path.dirname(__file__)))
	for folder_name in ('3rdParty', '3rdparty'):
		third_party_path = os.path.join(utils_path, folder_name)
		if os.path.isdir(third_party_path):
			return third_party_path

	third_party_path = os.path.join(utils_path, '3rdParty')
	os.makedirs(third_party_path, exist_ok=True)
	return third_party_path
...
def get_preferences():
	...
	for addon in bpy.context.preferences.addons.values():
		...
		if prefs is None:
			continue
		if hasattr(prefs, "texconv_path") or hasattr(prefs, "starfield_data_path"):
			return prefs
...
def get_preference(prop:str):
	...
	if prefs is None:
		return None
	return getattr(prefs, prop, None)

def get_texconv_path() -> str|None:
	prefs = get_preferences()
	if prefs is not None:
		texconv_path = getattr(prefs, "texconv_path", "")
		if texconv_path and os.path.exists(texconv_path) and os.path.isfile(texconv_path):
			return texconv_path

	for fallback_name in ("texconv.exe", "Texconv.exe"):
		fallback_path = os.path.join(ThirdPartyFolderPath(), fallback_name)
